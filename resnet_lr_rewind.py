...
for name,para in net.named_parameters():
    if ('conv' in name or 'shortcut.0' in name or 'linear' in name) and ('weight' in name):
        param_groups.append(para)

main_lr = [args.lr * (1 + math.cos((epoch + 1) * math.pi / args.epochs)) / 2 for epoch in range(args.epochs)]
layerwise_lr_idx = np.repeat(0, len(param_groups))
...
for epoch in range(start_epoch, start_epoch+args.epochs):
    ...
    if epoch > 0:
        alpha_ratios = np.divide(alpha_all,prev_epoch_alphas)
        layerwise_lr_idx[alpha_ratios>1.5]=0
    para_save=[]
    for i in range(n):
        para_save.append({'params': param_groups[i],
                          'lr': main_lr[layerwise_lr_idx[i]], 'momentum': 0.9,
                          'weight_decay': 5e-4})
    params_id = list(map(id, param_groups))
    other_params = list(filter(lambda p: id(p) not in params_id, net.parameters()))
    para_save.append({'params': other_params})
    optimizer = torch.optim.SGD(para_save, lr=main_lr[epoch], momentum=0.9, weight_decay=5e-4)

    prev_epoch_alphas = alpha_all
