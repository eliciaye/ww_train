...
#        return "{} v{} by {}\n{}\n{}".format(__name__, __version__, __author__, __description__, __url__)
    ...
    def combined_eigenvalues(self, Wmats, N, M, n_comp, params, sample_evals=False):
        ...
        for W in Wmats:
            ...
            W = W.astype(float)
            print("Running SVD on W={}:  W.shape={}  n_comp = {}".format(W, W.shape, n_comp))
            logger.debug("Running SVD:  W.shape={}  n_comp = {}".format(W.shape, n_comp))
            ...
            if sample_evals and params['sample_evals']: # we are sampling and alpha > 6
                sample_size = len(sv)//5
                new_svals = np.repeat(np.max(sv), sample_size) + np.random.normal(0, 1,sample_size)
                # if 0 < alpha < 2 :
                #     new_svals = np.repeat(np.min(sv), sample_size) + np.random.normal(0, 1,sample_size)
                #     sv = np.flip(sv)
                # else:
                #     new_svals = sv[:sample_size]
                # print("previous svals:", sv[:sample_size])
                # print("new svals:", new_svals)
                sv[:sample_size] = new_svals
                sv=np.sort(sv/np.max(sv))

                W = np.reshape(u @ (sv[..., None] * vh), W.shape)
                print(W)
    ...
    def analyze(self, model=None, layers=[], min_evals=0, max_evals=None, sample_evals=False,
        ...
        params['sample_evals'] = sample_evals
        params['plot'] = plot
        ...
        for ww_layer in layer_iterator:
            if not ww_layer.skipped and ww_layer.has_weights:
                ...
                if ww_layer.evals is not None:
                    ...
                    if params['sample_evals'] and ww_layer.get_value('alpha') > 6:
                        evals, sv_max, rank_loss = self.combined_eigenvalues(ww_layer.Wmats, ww_layer.N, ww_layer.M, ww_layer.num_components, params, sample_evals=True)
                        # Wmats updated with new svals for each W
                        ww_layer.evals = evals
                        ww_layer.set_column("num_evals", len(evals))
                        ww_layer.set_column("sv_max", sv_max)
                        ww_layer.set_column("rank_loss", rank_loss)
                        ww_layer.set_column("lambda_max", np.max(evals))
                        print("original weight shape:",ww_layer.layer.weight.shape)

                        # w = [np.array(ww_layer.layer.weight.data.clone().cpu())]
                        with torch.no_grad():
                            ww_layer.layer.weight.data = torch.from_numpy(np.reshape(ww_layer.Wmats, ww_layer.layer.weight.shape))
