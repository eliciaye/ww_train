...
for epoch in range(start_epoch, start_epoch+args.epochs):
    ...
    alpha_all=[]
    for i in range(details.shape[0]):
        alpha_all.append( details.loc[i, 'alpha'])

    for i in range(n // 2):
        para_copy.append({'params': net_para1[np.argsort(alpha_all)[i]],
                          'lr': (0.35 * args.lr + 0.15 * args.lr * 2 * i / n) * (
                                      1 + math.cos((epoch + 1) * math.pi / args.epochs)), 'momentum': 0.9,
                          'weight_decay': 5e-4})
    ...
    para1 = [value for index, value in sorted(list(zip(np.argsort(alpha_all), para_copy)), key=itemgetter(0))]

    params_id = list(map(id, net_para1))
    other_params = list(filter(lambda p: id(p) not in params_id, net.parameters()))
    para1.append({'params': other_params})
    optimizer = torch.optim.SGD(para1, lr=l, momentum=0.9, weight_decay=5e-4)
