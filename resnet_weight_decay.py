...
parser.add_argument('--function', default='softmax', type=str, help='using alpha')
parser.add_argument(
    '--seed',
    type=int,
    default=7,
    help='random seed')
parser.add_argument(
    '--early_stop_lw', action='store_true', help='Layerwise early stop.')
parser.add_argument(
    '--lr_schedule', action='store_true', help='Layerwise tune LR.')
parser.add_argument(
    '--sample_wd', action='store_true', help='Sample weight decays.')
...
for epoch in range(start_epoch, start_epoch+args.epochs):
    ...
    if args.early_stop_lw:
        alpha_all[alpha_all <=3] = 0
        
    if args.function == 'softmax':
        alpha_fn = np.exp(alpha_all)
    elif args.function == 'sqrt':
        alpha_fn = np.sqrt(alpha_all)
    elif args.function == 'cbrt':
        alpha_fn = np.cbrt(alpha_all)
    elif args.function == 'log2':
        alpha_fn = np.log2(alpha_all)
    elif args.function == 'log':
        alpha_fn = np.log(alpha_all)
    else:
        alpha_fn=alpha_all
        
    alpha_probs = alpha_fn/np.sum(alpha_fn)

    l = args.lr * (1 + math.cos((epoch + 1) * math.pi / args.epochs)) / 2
    if args.lr_schedule:
        for i in range(n):
            para_save.append({'params': net_para1[i],
                            'lr': l * n * alpha_probs[i], 'momentum': 0.9,
                            'weight_decay': 5e-4  * n * alpha_probs[i]})
    else:  
        if args.sample_wd:
            new_weight_decays=np.sort(np.random.normal(5e-4,1e-4,n))
            new_weight_decays = new_weight_decays - min(new_weight_decays)
            for i in range(n):
                para_save.append({'params': net_para1[sorted_alpha_idx[i]],
                                'lr': l, 'momentum': 0.9,
                                'weight_decay': new_weight_decays[i]})
        else:
            for i in range(n):
                para_save.append({'params': net_para1[i],
                                'lr': l, 'momentum': 0.9,
    ...
    para_save.append({'params': other_params})
    optimizer = torch.optim.SGD(para_save, lr=l, momentum=0.9, weight_decay=5e-4)
    print("layerwise weight decay: ",[optimizer.state_dict()['param_groups'][m]['weight_decay'] for m in range(n)])
    
    for m in range(n):
        reclr[m].append(optimizer.state_dict()['param_groups'][m]['lr'])
    print("layerwise learning rate: ",[reclr[m][-1] for m in range(n)])
...
print("Epoch {}, Test Acc = {}\nFinal Train Acc = {}\n".format(args.epochs,wwtestacc_final,wwtrainacc_final))
